...
type ProductODataHandler struct {
	repo Repository.Repository[Models.Product]
}

func NewProductODataHandler(repo Repository.Repository[Models.Product]) *ProductODataHandler {
	return &ProductODataHandler{repo: repo}
}
...
func (h *ProductODataHandler) GetAll(c *gin.Context) {
...
func (h *ProductODataHandler) GetByID(c *gin.Context) {
...
func (h *ProductODataHandler) Count(c *gin.Context) {
	conditions, err := ParseODataFilter(c.Query("$filter"))
	if err != nil {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: err.Error()})
		return
